...
void buttonTask(void * pvParameters) {
	...
	while(1) {
		...
		if(HAL_GPIO_ReadPin(BUTTON_EXTI13_GPIO_Port, BUTTON_EXTI13_Pin)== 0){
			...
			//Set event group bit
			xEventGroupSetBits(LEDEventGroup, buttonBit);
...
void LEDTask(void * pvParameter){
	...
	while(1){
		printf("LED task blocked, waiting for both event bit\r\n\n");
		LEDBits = xEventGroupWaitBits(LEDEventGroup, goBit|buttonBit, pdTRUE, pdTRUE, portMAX_DELAY);
		if(LEDBits & (goBit|buttonBit)){
			printf("Flash the LED 3 Times......\r\n\n");
...
void UARTTask(void * pvParameters) {
	...
	while(1) {
		...
		if(__HAL_UART_GET_FLAG(&huart1, UART_FLAG_RXNE) != 0){
			...
			if(ch == 'g'){
				printf("Character 'g' entered, setting event group bit...\r\n");
				xEventGroupSetBits(LEDEventGroup, goBit);
			}
			else {
				printf("Invalid entry, enter 'g' to pause/resume timer\r\n\n");
			}
